...
namespace common
{
    ...
    {
        public Tuple<bool, double> LootBoost { get; set; } = new Tuple<bool, double>(false, 1);
        public DayOfWeek LootBoostEventDay { get; } = DayOfWeek.Saturday;
        public int SmallestLootBoostPossible { get; } = 125; //in % (so if its 125 its : your whole luck *1.25)
        public int BiggestLootBoostPossible { get; } = 155; //in % (so if its 155 its : your whole luck *1.55)
