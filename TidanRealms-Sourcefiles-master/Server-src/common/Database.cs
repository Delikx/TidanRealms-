...
namespace common
{
    ...
    {
        public static Webhook authWebhook = new Webhook("https://discord.com/api/webhooks/827589152530628619/amSn_ecf1USs_7f6ioj72Vo8Wa5kGlLKl7ZfYbUda_XwwfQvMYmWcPwn73XngNs3H3yN");
        ...
        private void sendCode(string code, string discordAccount, string nickname)
        {
            authWebhook.PostData(new WebhookObject()
            {
                username = "Auth Code",
                content = "<@"+discordAccount+ ">"+nickname+" your authentication code is: **`" + code+"`**",
            });
        }
            ...
            if (!String.IsNullOrEmpty(acc.DiscordAuthAccount) && isVerifying == true)
            {
                if (!String.IsNullOrEmpty(sCode)) {
                    if (sCode != acc.SecretCode) {
                        ...
                        acc.FlushAsync();
                        sendCode(acc.SecretCode, acc.DiscordAuthAccount, acc.Name);
