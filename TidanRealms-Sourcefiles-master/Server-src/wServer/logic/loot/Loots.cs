    ...
    public class Loot : List<MobDrops>
    {
        ...
        public void Handle(Enemy enemy, RealmTime time)
        {
            ...
            foreach (var player in eligiblePlayers)
            {
                ...
                var eventBoost = eventBoostInfo.Item1 ? eventBoostInfo.Item2 : 1;
                ...
                foreach (var i in possibleDrops)
                {
                    if (player.Item2! >= enemy.MaximumHP / 200) // deal atleast 0.5% damage to qualify for loot.
                    {
                        ...
                        var LootChance = i.Probabilty * (1 + sizeBoost) * eventBoost; // Item Drop Chance * Solo Boss Drop LB which is (BOSSHP * 4) / DMG)
                        if (i.Item.Tier >= 0 && i.Item.Potion != true)// If item is tiered, dont drop loot modified with loot boost stats.
                        {
                            if (Rand.NextDouble() < LootChance * (1 + lootDropBoost))
                            {
                                loot.Add(i.Item);
                                reqDrops[i]--;
                            }
                        ...
                        {
                            if (Rand.NextDouble() < LootChance * (1 + (lootDropBoost + luckStatBoost + damagelootboost + DMGExtraLB))) //MAX: 50 + (50 - 145) + 100 + 25 = 225% increase to 325% increase
                            {
                                loot.Add(i.Item);
                                reqDrops[i]--;
                            }
                        }
                        else if (Rand.NextDouble() < LootChance * (1 + (lootDropBoost + luckStatBoost + DMGExtraLB)))
                        {
                            loot.Add(i.Item);
                            reqDrops[i]--;
                        }
