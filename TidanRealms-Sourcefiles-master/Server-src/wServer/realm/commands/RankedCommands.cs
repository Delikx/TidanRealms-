...
namespace wServer.realm.commands
{
    ...
    class MuteCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string args)
        {
            ...
            if (timeout < 0)
            {
                ...
                wServer.networking.webhooks.Webhooks.SendToDiscordAsMuteLog("Mute", "", $"**" + player.Name + $"**(" + player.AccountId + ") muted " + $"**" + acc.Name + $"**(" + acc.AccountId + ") indefinitely");
            }
            else
            {
                _manager.Database.Mute(acc.IP, TimeSpan.FromMinutes(timeout));
                player?.SendInfo(name + " successfully muted for " + timeout + " minutes.");
                wServer.networking.webhooks.Webhooks.SendToDiscordAsMuteLog("Mute", "", $"**" + player.Name + $"**(" + player.AccountId + ") muted " + $"**" + acc.Name + $"**(" + acc.AccountId + ") for " + timeout + " minutes.");
            }
        ...
        protected override bool Process(Player player, RealmTime time, string name)
        {
            ...
            player.Manager.Database.IsMuted(acc.IP).ContinueWith(t =>
            {
                ...
                if (t.Result)
                {
                    ...
                    wServer.networking.webhooks.Webhooks.SendToDiscordAsMuteLog("Unmute", "", $"**" + player.Name + $"**(" + player.AccountId + ") unmuted " + $"**" + acc.Name + $"**(" + acc.AccountId + ")");
                }
                else
                {
    ...
    class BanAccountCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string args)
        {
            ...
            player.SendInfo(!string.IsNullOrEmpty(bInfo.Name) ?
                ...
                "Ban successful.");
            wServer.networking.webhooks.Webhooks.SendToDiscordAsBanLog("Ban", "", $"**"+player.Name + $"**(" + player.AccountId + ") banned "+$"**" + acc.Name + $"**(" + acc.AccountId + ") for: " + bInfo.banReasons);
            return true;
        }
    ...
    class BanIPCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string args)
        {
            ...
            wServer.networking.webhooks.Webhooks.SendToDiscordAsBanLog("Ban", "", $"**" + player.Name + $"**(" + player.AccountId + ") banned (both account and ip) " + $"**" + acc.Name + $"**(" + acc.AccountId + ") for: " + reason);
            return true;
        }
    }
    ...
    class UnBanAccountCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string args)
        {
            ...
            if (banned && ipBanned)
            {
                ...
                wServer.networking.webhooks.Webhooks.SendToDiscordAsUnbanLog("Unban", "", $"**" + player.Name + $"**(" + player.AccountId + ") unbanned (account and ip) " + $"**" + acc.Name + $"**(" + acc.AccountId + ")");
                return true;
            }
            if (banned)
            {
                player.SendInfo($"Success! {acc.Name}'s account no longer banned.");
                wServer.networking.webhooks.Webhooks.SendToDiscordAsUnbanLog("Unban", "", $"**" + player.Name + $"**(" + player.AccountId + ") unbanned (account) " + $"**" + acc.Name + $"**(" + acc.AccountId + ")");
                return true;
            }

            player.SendInfo($"Success! {acc.Name}'s IP no longer banned.");
            wServer.networking.webhooks.Webhooks.SendToDiscordAsUnbanLog("Unban", "", $"**" + player.Name + $"**(" + player.AccountId + ") unbanned (ip) " + $"**" + acc.Name + $"**(" + acc.AccountId + ")");
            return true;
        }
    }
        ...
        protected override bool Process(Player player, RealmTime time, string args)
        {
            ...
            if(args.ContainsIgnoreCase("Loot Boost"))
            {
                ...
                if(arg.Length == 3)
                {
                    ...
                    try
                    {
                        ...
                        wServer.networking.webhooks.Webhooks.AnnounceOnDiscord("Cyberious", "<@&837755607540301854>", $"{player.Name} has hosted a loot boost event!");
                        return true;
                    }
                    catch { return false; }
                }
            ...
            if (args.ContainsIgnoreCase("Stat Boost"))
            {
                ...
                if (arg.Length == 4)
                {
                    try
                    {
                        ...
                        wServer.networking.webhooks.Webhooks.AnnounceOnDiscord("Cyberious", "<@&837755607540301854>", $"{player.Name} has hosted a stat boost event!");
                        return true;
                    }
                    catch { return false; }
                }
            ...
            if (args.ContainsIgnoreCase("Oryx Arena"))
            {
                ...
                if(player.Owner is Nexus)
                {
                    ...
                    wServer.networking.webhooks.Webhooks.AnnounceOnDiscord("Cyberious", "<@&837755607540301854>", $"{player.Name} has hosted a oryx arena event!");
                }
                else
                {
                ...
                if (arg.Length == 3)
                {
                    ...
                    try
                    {
                        if(arg[2].ContainsIgnoreCase("Legendary") || arg[2].ContainsIgnoreCase("Epic") || arg[2].ContainsIgnoreCase("Rare"))
                        {
                            ...
                            wServer.networking.webhooks.Webhooks.AnnounceOnDiscord("Cyberious", "<@&837755607540301854>", $"{player.Name} has hosted a {tier} Blood Moon event!");
